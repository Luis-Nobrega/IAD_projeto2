...
class MotionTrackingApp(QWidget):
    ...
    def detect_motion_objects(self, previous_frame, current_frame):
        if previous_frame is None:
            print("ERROR: Previous frame is None!")  # ✅ Check if previous_frame is missing
            return [], None, None
    ...
    def update_frame(self, fire=False):
        ...
        print(f"Memory Usage: {len(gc.get_objects())} objects in memory")
        ...
        if self.tracking_enabled and self.previous_frame is not None:
            ...
            print(f"Tracking {len(detected_objects)} objects")
    ...
    def follow_object(self, large, frame, fire=False):
        if large is None:
            print("ERROR: No large object to follow!")  # ✅ Check if large is None
            return
        ...
        print(f"Tracking Object at ({center_x}, {center_y})")

        if self.frame_counter % 10 == 0:
            print(f"Sending Command: Move to ({center_x}, {center_y})")
