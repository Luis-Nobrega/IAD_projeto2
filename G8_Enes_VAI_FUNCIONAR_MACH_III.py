...
class MotionTrackingApp(QWidget):
    def __init__(self):
        ...
        # Sliders absolutos para controlo "fino" lol
        ...
        self.servo_x_slider.valueChanged.connect(self.update_servo_x)
        ...
        self.servo_y_slider.valueChanged.connect(self.update_servo_y)
    ...
    def calibrate_motors(self):
        if ser:
            print("A enviar o comando de calibração dos motores...")
            ser.write(b"CALIBRAR\n")

    def update_servo_x(self, value):
        if ser:
            ser.write(f"SERVOX:{value}\n".encode())

    def update_servo_y(self, value):
        if ser:
