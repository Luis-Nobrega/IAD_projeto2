...
class MotionTrackingApp(QWidget):
    def __init__(self):
        ...
        self.down_button.clicked.connect(lambda: self.send_commands("[0,-1,0]"))
        self.left_button.clicked.connect(lambda: self.send_commands("[-1,0,0]"))
    ...
    def follow_object(self, large, frame, fire=False):
        ...
        # Inicializa posição atual se ainda não tiver sido definida
        if self.pos_x is None or self.pos_y is None:
            self.pos_x = center_frame_x
            self.pos_y = center_frame_y

        # Determina o centro do objeto detectado
        ...
        if center_x > self.pos_x + self.tolerancia:
            if abs(center_x - self.pos_x) > comprimento // 3:
                move_x = 3
            elif abs(center_x - self.pos_x) > comprimento // 6:
                move_x = 2
            else:
                move_x = 1

        elif center_x < self.pos_x - self.tolerancia:
            if abs(center_x - self.pos_x) > comprimento // 3:
                move_x = -3
            elif abs(center_x - self.pos_x) > comprimento // 6:
                move_x = -2
        ...
        # Decide movimento vertical
        if center_y > self.pos_y + self.tolerancia:
            if abs(center_y - self.pos_y) > altura // 3:
                move_y = 3
            elif abs(center_y - self.pos_y) > altura // 6:
                move_y = 2
            else:
                move_y = 1
        elif center_y < self.pos_y - self.tolerancia:
            if abs(center_y - self.pos_y) > altura // 3:
                move_y = -3
            elif abs(center_y - self.pos_y) > altura // 6:
                move_y = -2
        ...
        # Decide disparo
        if abs(center_x - self.pos_x) <= self.tolerancia and abs(center_y - self.pos_y) <= self.tolerancia and fire:
            shoot = 1

        self.send_commands(f"[{move_x},{move_y},{shoot}]")
